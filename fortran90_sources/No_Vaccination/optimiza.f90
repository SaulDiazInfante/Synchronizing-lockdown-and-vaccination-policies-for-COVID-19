...
include '../modulos/RK4_mod.f90'
  ...
  module par_mod
    ...
    real(8), parameter :: varepsilon = 1d0
    real(8), parameter :: kappa = 0.19607843d0
    real(8), parameter :: delta_h = 0.2d0
    real(8), parameter :: delta_v = 0d0!1d0/240d0 
    ...
    real(8), parameter :: lambda_v = 0d0!0.0006113521953813965d0
  ...
  contains
    ...
    function fob(x) result(fob_r)
      ...
      do i =1, npts
         cuV(i) = 0d0!qj(ct(i),rV)
         cuL(i) = qj(ct(i),rL)
      end do

      integ_aux = 0d0!(cuV + lambda_v)*(cL + cS + cE + cIa + cR)
    ...
    contains
      ...
      function Fvec(t, xvec) result(vecf_r)
        ...
        fS = (1d0 - th)*mu*Nstar + qj(t,rL)*xL + 0d0*delta_v*xV + delta_R* &
             xR - (lambda + 0d0*lambda_v + 0d0*qj(t,rV) + mu)*xS
        
        fE = lambda*(epsilon_par*xL + 0d0*(1d0 - varepsilon)*xV + xS) -    &
             (kappa + mu)*xE
        ...
        fV = 0d0
        !(lambda_v +  qj(t,rV))*xS - ((1d0 - varepsilon) * lambda +  &
             !delta_v + mu) * xV
  ...
  program test
    ...
    real(8), parameter, dimension(dimx) :: bL = -5d0*0.04
    real(8), parameter, dimension(dimx) :: bU = 5d0*0.04
