...
module par_mod
    ...
    real(8), parameter :: xcoverage = 0.85d0
    ...
    integer, parameter :: npRK = Tf * 1, dimf = 13, npts = npRK + 1
    !
...
module fob_mod
    ...
    !multiple of ncon
        ...
        !
            ...
            unf = sign(xvacT, 1.0d0)
            if(xvacT < 0.5d0 * xcoverage) return
            ...
            !The Fvec function for the ODE
                function Fvec(t, xvec) result(vecf_r)
                    ...
                    fV = abs((lambda_v +  u_Vt)) * (xS + xL) - &
                        ((1.0d0 - epsilon_par) * lambda +  delta_v + mu) * xV
                    fYIS = p * kappa * xE
                    fYIH = delta_h * xIS
                    fVcon =  abs((lambda_v +  u_Vt)) * (xL + xS + xE + xIA +xR)
                    fXMAY = 0.0d0
...
program main
    ...
    bL(1:dimr) = 0.0 * lambda_L 
    bL(dimr+1:dimx) = -lambda_v
    !Upper boudn of Lockdown and vaccination controls
    bU(1:dimr) = 1d0 * lambda_L
    bU(dimr+1:dimx) = 1d0 * lambda_v
